<template>
  <h2 class="intro-y text-lg font-medium mt-10">New Withdrawals Waiting Approval</h2>
          ...
          <form id="tabulator-html-filter-form" class="xl:flex sm:mr-auto">
            <div class="sm:flex items-center sm:mr-4">
              ...
              >
                ...
                <option value="holder">Account Holder Name</option>
          ...
          <form>
            <div class="input-form w-full mt-3">
              <div class="px-5 pb-8 text-center">
                <label for="crud-form-1" class="form-label">Reason</label>
                <input v-model.trim="validateWithdraw.reason.$model" id="crud-form-1" type="text" class="form-control w-full" placeholder="Reason/Remark" :class="{ 'border-danger': validateWithdraw.reason.$error }"/>
                <template v-if="validateWithdraw.reason.$error">
                  <div
                    v-for="(error, index) in validateWithdraw.reason.$errors"
                    :key="index"
  ...
  <div id="failed-notification-content" class="toastify-content hidden">
    <div class="flex w-full h-full">
      ...
      <div class="ml-4 mr-4">
        <div class="font-medium" id="message">There has been an Error Approving the Withdrawal. Please Try Again.</div>
      </div>
    </div>
  </div>
  ...
  <div id="failed-notification-content_fetch" class="toastify-content hidden">
    <div class="flex w-full h-full">
      <XCircleIcon class="text-danger" />
      <div class="ml-4 mr-4">
        <div class="font-medium" id="message">There has been an Error Fetching the Withdrawals. Please Try Again.</div>
      </div>
    </div>
  </div>
...
const withdrawal_data = reactive({
  reason:"",
})
...
const validateWithdraw = useVuelidate(rules, toRefs(withdrawal_data));

const approveRequest = async (app) => {
  //send request to the server for token
  loading.value = true;

  await backendApi.post("v1/approveWithdrawal", {
    approved: app,
...
const initializeTable = async () => {
  ...
  await backendApi.get('v1/withdrawalToApprove',  {
  },{
  headers: {
    'Authorization': "Bearer " + authStore.access_token,
    'Content-Type': 'multipart/form-data'
  ...
  tabulator.value = new Tabulator(tableRef.value, {
    ...
    printHeader: () => {
      ...
      return `<div class="w-full flex  justify-between items-center py-3 border-b-2">
                <h1 class="text-2xl font-bold ">Withdrawal Report</h1>
        ...
        formatter(cell) {
          const a = dom(`
                <a class="flex items-center mr-3 btn btn-success w-24" href="javascript:;" @click="deleteConfirmationModal.value = true; c_id.value = ${cell.getData().id}">
                  <i class="fa-solid fa-clipboard-check w-4 h-4 mr-1"></i> Approval</a>`);
